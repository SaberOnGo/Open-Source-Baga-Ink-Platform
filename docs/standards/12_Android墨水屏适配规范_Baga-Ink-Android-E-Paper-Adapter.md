...
6. 同一 `.ikp` 跨厂商运行；
7. 优先复用 Android / SQLite /成熟开源库；
8. 不因采用 library 就增加 Provider/Engine/Runtime 公共层；
9. SQLite 不经过 `baga.data`；
10. Automerge 可整体或拆模块复用，但不等于 `baga.sync`。
...
而不是让 IKP 假设厂商系统 SQLite 恰好满足某版本/扩展。
...
Automerge core 是 Baga 已采纳的 Local-first / CRDT 优先基础。

Android implementation MAY：

```text
使用完整 Rust core
使用 JS/WASM binding（如果该运行环境合理）
使用 Java/其他 binding
通过 automerge-c
...
当前 developer-facing Lua binding 尚未冻结，因此 IKP 不应该依赖一个 Baga 自造的 `baga.automerge`。

---

# 11. `baga.sync`

Android 上 `baga.sync` 仍只定义：
...
- 让所有 APK 变成 IKP；
- 强迫厂商统一刷新 SDK；
- 向 App 暴露 Android Context；
- 为 SQLite 再建 `baga.data`；
- 因 Automerge 优秀就强制整套 automerge-repo；
- 为每个成熟库创建公共 Provider/Engine 层。
