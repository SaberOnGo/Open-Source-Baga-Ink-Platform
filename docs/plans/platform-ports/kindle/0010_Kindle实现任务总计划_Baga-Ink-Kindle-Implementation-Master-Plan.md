...
Kindle 只是未来众多设备/OS 家族中的第一个完整 Platform Port。当前正式的物理任务目录规则以：
...
K0–K7 是长期 Milestone / Roadmap 语言，不直接对应物理 `K0_*`、`K1_*` 任务目录。真正施工时使用：
...
# 2. Task Design 与 AI Execution Prompt

本文件是 Kindle Port 的**长期总计划**，不直接承载每次施工的全部细步骤。

每次准备实现一个明确功能、模块、测试、调试、验证或真机目标时：

```text
先在 task/ 建立 Task Design
        ↓
形成 v001 / v002 / ...
        ↓
选定精确 Task Version
        ↓
AI 再在 execution-prompts/ 镜像目录中拆分具体执行步骤
```
...
不得因为 K0–K7 的逻辑编号而自行创建平行的 `K0_*`、`K1_*`、`prompt/`、`handoff/`、`scratch/` 等目录。
...
这些是逻辑执行顺序；真正创建实施文档时仍必须先建立 `task/<NNNN_...>/vNNN`，再由对应版本生成 `execution-prompts/`。
