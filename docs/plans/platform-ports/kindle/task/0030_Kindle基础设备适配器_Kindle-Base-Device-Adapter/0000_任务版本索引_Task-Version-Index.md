...
`v002` 进一步固定 Kindle 第一阶段的语言与 backend 选择，避免后续执行 Agent 在每个 subsystem 开工时重新进行语言选型。
...
|---|---|---|---|
| `v001` | Superseded | Base Adapter：probe/profile/quirk/capability/display/input/storage/lifecycle/power/self-test | 首个 Kindle Adapter 任务设计 |
| `v002` | **Selected** | 固定 Lua-first Kindle glue、KOReader wrapper、FBInk native reuse、混合语言边界与偏离 Gate | 将实现语言/绑定从临时决策变成版本化任务设计输入 |
...
```text
TASK-0010 current K0 Base Contract baseline available
TASK-0020 current K1 pinned kindlehf substrate bring-up evidence available
```
...
真机证据若要求改变 profile/quirk 分层、Base subsystem backend、默认语言/绑定矩阵或 Gate，应创建 `v003`；Execution Prompt 不得静默改变这些设计。
